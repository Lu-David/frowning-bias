...
Visualize train/val loss/acc curves.
"""
...
def main():
    ...
    # Visualize
    per_iteration = {"train_loss": [], "train_acc": []}
    per_epoch = {"train_loss": [], "train_acc": [], "val_loss": [], "val_acc": []}
    ...
    for epoch in range(model_args['max_epochs']):
        ...
        for x, y, fns, attrs in trainLoader:
            ...
            # Visualization stats
            with torch.no_grad():
                per_iteration["train_loss"].append(loss.item())
                ycpu = y.to('cpu').numpy()
                ycpu = np.argmax(ycpu,axis=1)
                yhatcpu = yhat.to('cpu').numpy()
                yhatcpu = np.argmax(yhatcpu,axis=1)
                per_iteration["train_acc"].append(
                    np.sum(ycpu==yhatcpu) / np.shape(ycpu)[0]
                )
        ...
        # Visualization stats
        per_epoch['train_loss'].append(train_loss)
        per_epoch['train_acc'].append(train_log['acc']['train'][-1])
        per_epoch['val_loss'].append(val_loss)
        per_epoch['val_acc'].append(train_log['acc']['val'][-1])
    ...
    Visualize
    """
    plt.plot(list(range(1,len(per_epoch["train_loss"])+1)), per_epoch['train_loss'], label="train_loss")
    plt.plot(list(range(1,len(per_epoch["val_loss"])+1)), per_epoch['val_loss'], label="val_loss")
    plt.title("Loss over Epochs")
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
    plt.legend()
    plt.show()
    plt.plot(list(range(1,len(per_epoch["train_acc"])+1)), per_epoch['train_acc'], label="train_acc")
    plt.plot(list(range(1,len(per_epoch["val_acc"])+1)), per_epoch['val_acc'], label="val_acc")
    plt.title("Accuracy over Epochs")
    plt.xlabel("Epochs")
    plt.ylabel("Accuracy")
    plt.legend()
    plt.show()
    plt.plot(list(range(1,len(per_iteration["train_loss"])+1)), per_iteration["train_loss"], label="train_loss")
    plt.title("Loss over Iterations")
    plt.xlabel("Iterations")
    plt.ylabel("Loss")
    plt.legend()
    plt.show()
    plt.plot(list(range(1,len(per_iteration["train_acc"])+1)), per_iteration["train_acc"], label="train_acc")
